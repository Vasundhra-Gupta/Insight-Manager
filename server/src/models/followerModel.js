...
export class SQLfollowers extends Ifollowers {
    async getFollowers(channelId) {
        try {
            ...
            const q = `SELECT u.user_id, u.user_name, u.user_firstName, u.user_lastName, user_avatar, ${q1} FROM followers f, users u WHERE f.follower_id = u.user_id AND f.following_id = ?`;
    ...
    async getFollowings(channelId) {
        try {
            ...
            const q = `SELECT u.user_id, u.user_name, u.user_firstName, u.user_lastName, user_avatar, ${q1} FROM followers f, users u WHERE f.following_id = u.user_id AND f.follower_id = ?`;
    ...
    async toggleFollow(userId, channelId) {
        try {
            ...
            return { message: "FOLLOWED_SUCCESSFULLY" };
        } catch (err) {
            throw new Error(err);
        }
    }
