...
export default function LoginPopup() {
    const { showLoginPopup, loginPopupText, setShowLoginPopup } = usePopupContext();
    const ref = useRef();
        ...
        showLoginPopup && (
            ...
            >
                <div className="relative bg-white text-black p-6 flex flex-col items-center justify-center gap-4">
                    <Button
                        btnText={<div className="size-[20px] stroke-black">{icons.cross}</div>}
                        onClick={() => setShowLoginPopup(false)}
                        className="absolute top-2 right-2"
                    />

                    <div>
                        <div className="text-xl font-medium">Login to {loginPopupText}</div>
                        <motion.hr
                            initial={{ width: 0 }}
                            animate={{ width: "100%" }}
